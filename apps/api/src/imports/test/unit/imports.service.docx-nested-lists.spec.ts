...
describe('ImportsService docx nested lists', () => {
  it('creates page content with nested list nodes', async () => {
    ...
    (mammoth as unknown as { convertToMarkdown: jest.Mock }).convertToMarkdown.mockResolvedValue({
      value: ['* 登录功能', '', '+ 用户通过手机号+密码登录', '+ 支持找回密码', '', '* 注册功能', '', '+ 用户通过手机号注册', ''].join('\n'),
    });
    ...
    const items = content[0]?.children as unknown as Array<Record<string, unknown>>;
    expect(items).toHaveLength(2);
    const firstItem = items[0];
    const nested = (firstItem?.children as unknown as unknown[]).find((c) => {
